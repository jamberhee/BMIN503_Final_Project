...
Due to a high level of missingness (\~60%) in language variables, I decided to stratify by primary language in a subanalysis instead of the entire analytic population.
...
Table 1 shows selected characteristics of the U.S. adults examined in our analysis divided by communication difficulty. We examined N=28,444 respondents, representing a weighted N=248,826,036 individuals. 
...
### Subgroup Analysis
Table 2 shows selected characteristics of the U.S. adults examined in our analysis divided by communication difficulty. We examined N=28,444 respondents, representing a weighted N=248,826,036 individuals. 

```{r}
table1_data_sub <- subset(data, complete.cases(comdiff,hip_binary,sex,
                                race,age,hearing,dementia,
                                fatigue,anxiety,stroke,lang_match))
## set var names
label(table1_data_sub$sex) <- "Sex"
label(table1_data_sub$age) <- "Age (years)"
label(table1_data_sub$race) <- "Race and Ethnicity"
label(table1_data_sub$dementia) <- "Dementia"
label(table1_data_sub$fatigue) <- "Fatigue"
label(table1_data_sub$anxiety) <-  "Anxiety"
label(table1_data_sub$stroke) <-  "Stroke"
label(table1_data_sub$hearing) <-"Hearing"

## create strata for table

## generate table1
table2 <- table1(~ sex + age + race + hearing + dementia +
                   fatigue + anxiety + stroke | lang_match*comdiff,
                 data=table1_data_sub,
