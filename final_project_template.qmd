...
weighted_data <- svydesign(id=~psu, weights=~sweight,
                          ...
                          data = data)
sum(weights(weighted_data))
weighted_sub <- survey::subset(weighted_data, complete.cases(comdiff,hip_binary,sex,
...
test <- as_tibble(summary(fit3_sub)$coef) %>% 
...
test_match <- as_tibble(summary(fit3_sub_match)$coef) %>% 
  mutate(or = exp(Estimate),
         lower.CI = exp(Estimate - 1.96*`Std. Error`),
         upper.CI = exp(Estimate + 1.96*`Std. Error`))
test_nomatch <- as_tibble(summary(fit3_sub_nomatch)$coef) %>% 
  mutate(or = exp(Estimate),
         lower.CI = exp(Estimate - 1.96*`Std. Error`),
         upper.CI = exp(Estimate + 1.96*`Std. Error`))
