...
adult2023 <- adult2023 |>
  mutate(hip_binary = ifelse(adult2023$PAIWKLM3M_A == "Never" | adult2023$PAIWKLM3M_A == "Some", 0, 1))
...
svytable(~comdiff+pain_binary, weighted_data_test)
```
...
data <- data %>% 
  rename(lang_other = LANGHM_A,
         ...
         anxiety = ANXEV_A,
         sex = SEX_A) %>% 
  select(id,strata,sweight,psu,comdiff,pain,hip,pain_binary,
         lang_other,lang_tv,lang_doc,lang_soc,race,age,
         hearing,dementia,fatigue,anxiety,sex)

cols <- c(6,9:12,15:19)
data[data[, cols] > 7, cols] <- NA
data[,10:12] <- sapply(data[,10:12], factor,
                       levels = c(1,2,7,8,9),
                       labels = c("English", "Other language",
                                  "Refused", "Not Ascertained",
                                  "DK"))
data[,6:7] <- sapply(data[,6:7], factor,
                       levels = c(1:4,7:9),
                       labels = c("Never", "Some", "Most", "Every",
                                  "Refused", "Not Ascertained",
                                  "DK"))
...
data <- data %>% 
  mutate(id = factor(lang),
         hip_binary =
         lang_other = factor(lang_other,
                             levels = c(1,2,7,8,9),
